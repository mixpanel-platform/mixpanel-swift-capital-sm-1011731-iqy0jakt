...
params).done(function(bounceResults){
  console.log("bounce results", bounceResults)
  ...
  params).done(function(nonBounceResults){
    console.log('non bounce results', nonBounceResults)
    ...
    _.each(campaignlist, function(values, key){     //start creating a object in the correct format be accepted by MPChart
      graphingData[values] = {}
    })
    ...
    _.each(bounceResults, function (bounceValue, bounceKey) {            
      _.each(nonBounceResults, function(nonBounceValue, nonBounceKey){
        if(bounceValue.key[0] === nonBounceValue.key[0] && bounceValue.key[1] === nonBounceValue.key[1]){ // if the campaign name and the date match calculate the bounce rate
          bounceRate = parseFloat(((bounceValue.value/nonBounceValue.value)*100).toFixed(2))   
  ...
  console.log("monthly bounce rate", monthlyData)
  console.log("calculated bounce rate", graphingData)
